...
include("/var/www/htmlphp/api/hamstring/class/Classes/PHPExcel.php"); 
...
switch ($action) {
	case 'test_func':
		...
		//exit;
		break;
	case 'export_to_excel_delete':
		if(!empty($data)){
		 	$return = $objFunctions->deleteReportFile($data);
		 }
		 else{ 
	...
	case 'export_to_excel':
		if(!empty($data)){
		 	$return = $objFunctions->exportToExcel($data);
		 }
		 else{ 
		 	$return = array('success'=>'0','message'=>CANNOT_BLANK);
		 }
